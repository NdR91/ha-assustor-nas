...
This document describes all configuration options and settings available in the Home Assistant Asustor NAS custom integration custom integration.
...
2. Find "Home Assistant Asustor NAS custom integration"
3. Click **Configure**
4. Modify settings
5. Click **Submit**
...
### `ha_asustor_nas_custom_integration.example_service`
...
```yaml
service: ha_asustor_nas_custom_integration.example_service
target:
  ...
  - alias: "Call service at sunset"
    trigger:
      - trigger: sun
        event: sunset
    action:
      - action: ha_asustor_nas_custom_integration.example_service
        target:
          entity_id: switch.device_name_switch
        data:
          parameter: "mode"
...
```yaml
blueprint:
  name: Home Assistant Asustor NAS custom integration Alert
...
- [GitHub Issues](https://github.com/NdR91/ha-assustor-nas/issues) - Report problems
