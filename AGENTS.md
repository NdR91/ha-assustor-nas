...
- **Repository:** NdR91/ha-assustor-nas

**Key directories:**

- `custom_components/ha_asustor_nas_custom_integration/` - Main integration code
...
- Integration logs: `custom_components.ha_asustor_nas_custom_integration: debug` in `config/configuration.yaml`
...
- Use the domain `ha_asustor_nas_custom_integration` for all DOMAIN references
- Prefix all integration-specific classes with `HomeAssistantAsustorNASCustom`
- Use "Home Assistant Asustor NAS custom integration" as the display title
...
- Inherit from platform base + `HomeAssistantAsustorNASCustomEntity`
